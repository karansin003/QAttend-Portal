...
contactAdminForm?.addEventListener("submit", async event => {
    ...
    if (!name || !email || !message) {
        contactAdminMessage.textContent = "Please fill all required fields.";
    ...
    contactAdminMessage.textContent = "Saving request and sending email to Admin...";
    contactAdminMessage.className = "form-feedback";

    const subject = `QAttend Access Request - ${name}`;
    const body =
    ...
    try {
        ...
        console.error("Contact Admin Firestore error:", error);
    }
    ...
    if (firestoreSaved && emailSent) {
        contactAdminMessage.textContent = "Request sent successfully. Admin has received the email and the request is visible in Admin → Requests.";
        contactAdminMessage.className = "form-feedback success";
        contactAdminForm.reset();
        setTimeout(() => closeLoginModal(contactAdminModal), 1200);
    } else if (firestoreSaved && !emailSent) {
        contactAdminMessage.textContent = `Request saved in Admin → Requests, but email could not be sent. ${emailError?.message || "Check EmailJS configuration."}`;
        contactAdminMessage.className = "form-feedback error";
    } else if (!firestoreSaved && emailSent) {
        contactAdminMessage.textContent = `Email sent to Admin, but the request could not be saved in Admin → Requests. Check Firestore Rules. ${firestoreError?.message || ""}`;
        contactAdminMessage.className = "form-feedback error";
        contactAdminForm.reset();
    } else {
        contactAdminMessage.textContent = `Request could not be saved or emailed. ${firestoreError?.message || emailError?.message || "Please try again."}`;
        contactAdminMessage.className = "form-feedback error";
    }
});
        ...
        rows.forEach(r=>{
            ...
            const title = r.type?.includes("student")
                ...
                : r.type?.includes("subject")
                    ...
                    : r.type === "add_section" ? "Add New Section"
                        : r.type === "contact_admin" ? "Contact Admin"
                        : "Request";
    ...
    try{
        ...
        if(!filtered.length){
            ...
            filtered.forEach(r=>{
                ...
                const title = r.type === "add_student" ? "Add Student" : r.type === "delete_student" ? "Delete Student" : r.type === "add_subject" ? "Add Subject" : r.type === "delete_subject" ? "Delete Subject" : r.type === "add_section" ? "Add New Section" : r.type === "contact_admin" ? "Contact Admin" : friendlyRequestType(r.type);
...
function openRequestModal(requestType="add_student"){
    if(profile?.role !== "cr") return;
    ...
    if(type) type.value=requestType;
    syncRequestForm();
}
